...
"""
To install adafruit_bno055 and board library, run:
...
class ImuPublisher(Node): 
    def __init__(self):
        ...
        # change topic from /imu/data to /imu as ekf looks for imu data in /imu topic or remap in launch file
        # self.publisher_ = self.create_publisher(Imu, '/imu', 10)    

        # Initialize the timer for periodic publishing (10 Hz)
        # self.timer = self.create_timer(0.1, self.publish_imu_data)

        # Initialize the timer for periodic publishing (20 Hz)
        ...
        self.offsets_gyroscope = (-1, -2, -1)
        self.offsets_accelerometer = (12, 30, -46)
        # mag offsets are environment specific, may need to be recalibrated if environment changes.
        self.offsets_magnetometer = (-49, -66, -637)
    ...
    def publish_imu_data(self):
        ...
        # Check calibration levels (0-3)
        # sys, gyro, accel, mag = self.sensor.calibration_status
        # self.get_logger().info(f"Calibration Status: Sys={sys}, Gyro={gyro}, Accel={accel}, Mag={mag}")
        # # 0 (uncalibrated) to 3 (fully calibrated).
        # if sys < 3 or gyro < 3 or accel < 3 or mag < 3:
        #     self.get_logger().warn("IMU not fully calibrated")
